...
import { MdCheckCircle, MdArrowForward, MdImage, MdUploadFile, MdClose, MdEdit, MdCheck } from 'react-icons/md';
import { FaWhatsapp } from 'react-icons/fa';

export const STOCK_CYCLING_IMAGES = [
  { id: '1', title: 'Oficina & Manutenção', url: 'https://images.unsplash.com/photo-1485965120184-e220f721d03e?auto=format&fit=crop&w=1200&q=80' },
  { id: '2', title: 'Mountain Bike / Trilha', url: 'https://images.unsplash.com/photo-1544197150-b99a580bb7a8?auto=format&fit=crop&w=1200&q=80' },
  { id: '3', title: 'Ciclismo de Estrada', url: 'https://images.unsplash.com/photo-1517649763962-0c623266010b?auto=format&fit=crop&w=1200&q=80' },
  { id: '4', title: 'Bike Urbana & Estilo', url: 'https://images.unsplash.com/photo-1507035895480-2b3156c31fc8?auto=format&fit=crop&w=1200&q=80' },
  { id: '5', title: 'Equipamentos & Acessórios', url: 'https://images.unsplash.com/photo-1532298229144-0ec0c57515c7?auto=format&fit=crop&w=1200&q=80' },
  { id: '6', title: 'Showroom de Loja', url: 'https://images.unsplash.com/photo-1571068316344-75ad7692d490?auto=format&fit=crop&w=1200&q=80' },
];

export default function WysiwygPreview({ blocks, onUpdateBlocks, store }) {
  ...
  // Handle Local File Upload Convert to DataURL / URL
  ...
  return (
    <div className="bg-slate-950 rounded-2xl border border-slate-800 text-slate-100 overflow-hidden shadow-2xl relative">
            ...
            case 'HERO':
              return (
                <section key={bIdx} className="text-center py-12 px-4 bg-gradient-to-b from-slate-900 to-slate-950 rounded-3xl border border-slate-800 relative overflow-hidden space-y-6">
                  ...
                  {block.imageUrl ? (
                    <div className="relative h-64 rounded-2xl overflow-hidden mb-6 border border-slate-800 group">
                      <img src={block.imageUrl} alt="Hero" className="w-full h-full object-cover" />
                      <button
                        onClick={() => setEditingImageIdx(bIdx)}
                        className="absolute inset-0 bg-slate-950/60 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-2 text-white font-bold text-sm"
                      >
                        <MdImage className="w-5 h-5 text-emerald-400" />
                        Trocar Foto do Destaque
                      </button>
                    </div>
                  ) : (
                    <div
                      onClick={() => setEditingImageIdx(bIdx)}
                      className="h-32 border-2 border-dashed border-slate-800 hover:border-emerald-500/50 rounded-2xl flex items-center justify-center gap-2 text-slate-400 hover:text-emerald-400 cursor-pointer transition text-xs font-semibold"
                    >
                      <MdImage className="w-5 h-5" />
                      Clique aqui para adicionar uma foto de destaque ao Hero
                    </div>
                  )}
                  ...
                  <div className="max-w-3xl mx-auto space-y-4">
                    ...
                    <div className="relative group">
                      ...
                      <span className="text-[10px] text-slate-500 block text-right pr-2">Clique no texto para editar</span>
                    </div>
      ...
      {/* Modal Selection for 3 Image Suggestions + Upload */}
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-2xl w-full p-6 space-y-5 shadow-2xl relative text-white">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold">Escolher Imagem para o Bloco</h3>
                <p className="text-xs text-slate-400">Selecione uma imagem da galeria recomendada ou faça o upload de um arquivo</p>
              </div>
            </div>
