...
export default function CustomerDetailPage() {
  ...
  const fetchCustomerData = async () => {
    try {
      setLoading(true);
      const [customerRes, warrantiesRes, eventsRes, storesRes] = await Promise.all([
      ...
      // Mock de seguros ativos (substituir por API real)
      setInsurances([
        {
          id: '1',
          productName: 'Apex Pro 2024',
          startDate: '2024-01-15',
          endDate: '2025-01-15',
          status: 'ACTIVE',
          coverage: 'Roubo, Furto e Danos Acidentais'
        }
      ]);

      // Mock de sugestões de produtos (substituir por API real)
      setProducts([
        {
          id: '1',
          name: 'Pneu Continental Grand Prix 5000',
          category: 'Pneus',
          price: 'R$ 289,90',
  ...
  const tabs = [
    ...
    { id: 'products', label: 'Produtos Sugeridos' },
            ...
            {activeTab === 'insurances' && (
              <div>
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-semibold">Seguros Ativos</h3>
                  <Link
                    to="/seguro"
                    className="text-primary hover:underline text-sm font-medium"
                  >
                    + Solicitar Novo Seguro
                  </Link>
                </div>
                {insurances.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-gray-600 mb-4">Nenhum seguro ativo</p>
                    <Link
                      to="/seguro"
                  ...
                  <div className="space-y-4">
                    {insurances.map((insurance) => (
                      <div key={insurance.id} className="border border-gray-200 rounded-lg p-4">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="font-semibold">{insurance.productName}</p>
                            <p className="text-sm text-gray-600">
                              Cobertura: {insurance.coverage}
                            </p>
                            <p className="text-sm text-gray-600">
                              Vigência: {new Date(insurance.startDate).toLocaleDateString('pt-BR')} até{' '}
            ...
            {activeTab === 'products' && (
              <div>
                ...
                <p className="text-gray-600 text-sm mb-6">
                  Sugestões personalizadas baseadas nas bikes do cliente
                </p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {products.map((product) => (
                    <div key={product.id} className="border border-gray-200 rounded-lg p-4">
                      ...
                      <button className="w-full px-4 py-2 border border-primary text-primary rounded-lg hover:bg-primary hover:text-white transition-all">
                        Ver Detalhes
                      </button>
                    </div>
                  ))}
