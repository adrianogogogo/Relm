...
export class WarrantyService {
  ...
  private async generateAutoTasks(
    ...
    // Tradução de roles para rótulos amigáveis
    ...
    const taskData = templates.map((tpl) => {
      const label = roleLabels[tpl.targetRole] || tpl.targetRole;
      const userName = roleNameMap[tpl.targetRole];
      const assignee = userName ? `[${label}] ${userName}` : `[${label}]`;
