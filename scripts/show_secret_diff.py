...
# Script remoto: para cada chave de interesse, le do backup (antigo) e do .env vivo (novo)
# e imprime apenas IMPRESSAO DIGITAL (6 primeiros + 4 ultimos + tamanho). NAO imprime valor completo.
REMOTE_PY = r'''
def parse(path):
    d = {}
    try:
...
for k in keys:
    ov = old.get(k)
    nv = new.get(k)
    # Para DATABASE_URL, mascarar so a senha (mostrar se mudou)
    print(k)
    print("  ANTIGO: " + fp(ov))
    print("  NOVO  : " + fp(nv))
