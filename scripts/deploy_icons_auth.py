...
# backend: upload src + build + reload (fix de auth ac3a4e0)
upload(LOCAL_SRC, BE+'/src')
out,err=run(c,'cd %s && NODE_ENV=production npx prisma generate 2>&1 | tail -1 && npm run build 2>&1 | tail -3'%BE)
sys.stdout.buffer.write(('2) backend build:\n'+out+err).encode())
out,err=run(c,'pm2 reload relm-careplus-prod-backend && pm2 save 2>&1 | tail -1')
sys.stdout.buffer.write(('3) pm2 reload:\n'+out[-200:]+'\n').encode())

# frontend: publica SOBREPONDO (preserva assets antigos p/ abas abertas nao quebrarem)
